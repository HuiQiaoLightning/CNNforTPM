...
class parameters(object):
    def __init__(self, Nx, Ny, Nz):
        ...
        #self.kx = self.dkx * np.arange(-self.Nx / 2 + 1, self.Nx / 2 + 1)
        #self.ky = self.dky * np.arange(-self.Ny / 2 + 1, self.Ny / 2 + 1)

        # phase factor for Fresnel diffrantion
        [Kx, Ky] = np.meshgrid(self.kx, self.ky)
        K2 = Kx ** 2 + Ky ** 2
        temp = (self.k ** 2 - K2).astype(np.complex)
        #index = np.where(K2[:,:] < self.k0 ** 2)
        #K2[index] = 0
        ...
        self.prop_window = np.exp(-((X ** 2 / (0.7 * self.Lx / 2) ** 2 + Y ** 2 / (0.8 * self.Lx / 2) ** 2)) ** 10)


demo = parameters(256, 256, 160)